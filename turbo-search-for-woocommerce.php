...
// OTSW\Search class namespace (Pro is a superset of this edition's core). If
// both plugin files load in the same request, PHP fatals with "Cannot
// redeclare otsw_search_init()" — and NOT because of some runtime race: an
// ordinary top-level `function otsw_search_init(){}` (not nested inside an
...
// only WP core functions and a literal basename — never the OTSW_*
...
// otsw_deny_activation_if_pro_active() below closes that gap for the
...
if ( is_plugin_active( $otsw_pro_edition_basename ) ) {
	...
	add_action( 'admin_notices', static function (): void {
		...
		<div class="notice notice-warning is-dismissible">
			<p>
				<?php esc_html_e( 'Turbo Search for WooCommerce: the free edition cannot run alongside Pro and has been deactivated automatically.', 'ozulabs-turbo-search-for-woocommerce' ); ?>
			</p>
		</div>
		<?php
	} );
 ...
 * for the same compile-time-binding reason as otsw_search_init() below.
 */
if ( ! function_exists( 'otsw_deny_activation_if_pro_active' ) ) {
	function otsw_deny_activation_if_pro_active(): void {
		if ( ! function_exists( 'is_plugin_active' ) ) {
			require_once ABSPATH . 'wp-admin/includes/plugin.php';
		}
		...
		deactivate_plugins( plugin_basename( __FILE__ ) );
		wp_die(
			esc_html__( 'Turbo Search for WooCommerce (Free) was not activated because Turbo Search for WooCommerce Pro is already active on this site — Pro already includes everything Free does. Deactivate Pro first if you need to switch to Free.', 'ozulabs-turbo-search-for-woocommerce' ),
			esc_html__( 'Plugin activation blocked', 'ozulabs-turbo-search-for-woocommerce' ),
...
/**
 * Autoloader for OTSW classes.
 * Maps OTSW\Search\ClassName to includes/class-class-name.php
 */
spl_autoload_register( function ( string $class_name ): void {
	$prefix   = 'OTSW\\Search\\';
	$base_dir = OTSW_PLUGIN_DIR . 'includes/';
 ...
 */
if ( ! function_exists( 'otsw_search_init' ) ) {
	function otsw_search_init(): void {
		// Check WooCommerce dependency.
		if ( ! class_exists( 'WooCommerce' ) ) {
			add_action( 'admin_notices', 'otsw_woocommerce_missing_notice' );
			return;
		}

		// Wait for the class to be available (it will be built in Phase 2)
		if ( class_exists( '\\OTSW\\Search\\Activator' ) ) {
			\OTSW\Search\Activator::init();
		}
		if ( class_exists( '\\OTSW\\Search\\Indexer' ) ) {
			\OTSW\Search\Indexer::init();
		}
		if ( class_exists( '\\OTSW\\Search\\Search_Handler' ) ) {
			\OTSW\Search\Search_Handler::init();
		}
		if ( class_exists( '\\OTSW\\Search\\Frontend' ) ) {
			\OTSW\Search\Frontend::init();
		}
		if ( class_exists( '\\OTSW\\Search\\Admin_Settings' ) ) {
			\OTSW\Search\Admin_Settings::init();
		}
	}
}
add_action( 'plugins_loaded', 'otsw_search_init' );
 ...
 * otsw_search_init() above.
 */
if ( ! function_exists( 'otsw_woocommerce_missing_notice' ) ) {
	function otsw_woocommerce_missing_notice(): void {
		?>
	<div class="notice notice-error is-dismissible">
		<p><?php esc_html_e( 'Turbo Search for WooCommerce requires WooCommerce to be installed and active.', 'ozulabs-turbo-search-for-woocommerce' ); ?></p>
	</div>
		<?php
	}
}

// Register activation hooks. otsw_deny_activation_if_pro_active() is
...
register_activation_hook( __FILE__, 'otsw_deny_activation_if_pro_active' );
register_activation_hook( __FILE__, array( '\\OTSW\\Search\\Activator', 'activate' ) );
register_deactivation_hook( __FILE__, array( '\\OTSW\\Search\\Activator', 'deactivate' ) );
