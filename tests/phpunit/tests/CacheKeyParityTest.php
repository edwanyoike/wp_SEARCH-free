...
final class CacheKeyParityTest extends TestCase {
	...
	protected function setUp(): void {
		...
		$GLOBALS['otsw_test_active_plugins'] = array( 'ozulabs-turbo-search-for-woocommerce/turbo-search-for-woocommerce.php' );

		update_option( 'woocommerce_currency', 'USD' );
		update_option( 'otsw_cache_version', 3 );
	...
	private function interceptedKey(): ?string {
		otsw_cache_bypass_intercept();
		$reads = $GLOBALS['otsw_test_transients']['reads'] ?? array();
	...
	private function scopedRlKey( string $ip ): string {
		return Query_Normalizer::site_scope() . '_otsw_rl_' . md5( $ip );
	}
	...
	public function test_rate_limiter_is_consulted_before_serving_from_cache(): void {
		...
		$this->assertSame( 1, $GLOBALS['otsw_test_rate_limits'][ $expected_key ]['hits'] ?? null );

		// A second call within the window must increment the same counter,
		// not reset it or use a different key.
		$this->interceptedKey();
		$this->assertSame( 2, $GLOBALS['otsw_test_rate_limits'][ $expected_key ]['hits'] ?? null );
	}
	...
	// regardless of the otsw_rate_limit_requests/window options an
	...
	// otsw_cache_bypass_intercept() calls `exit` directly the moment
	...
	public function test_configured_max_above_sixty_does_not_reject_the_sixty_first_request(): void {
		update_option( 'otsw_rate_limit_requests', 200 );
		$GLOBALS['otsw_test_rate_limits'][ $this->scopedRlKey( '' ) ] = array(
		...
		$this->assertSame( 61, $GLOBALS['otsw_test_rate_limits'][ $this->scopedRlKey( '' ) ]['hits'] ?? null );
	}
	...
	public function test_configured_window_is_passed_through_unchanged(): void {
		...
		// unchanged proves the MU path reads otsw_rate_limit_window rather than
		...
		$this->assertSame( $expected_window_start, $GLOBALS['otsw_test_rate_limits'][ $expected_key ]['window_start'] ?? null );
	}

	public function test_default_behavior_remains_sixty_requests_per_sixty_seconds(): void {
		// No otsw_rate_limit_requests/window option set — must match the
