 ...
 * cleanup must never regress to broad LIKE 'otsw_%' patterns (which would
 ...
 * update_option( 'otsw_new_thing', ... ) anywhere fails this suite until
...
final class CleanupCoverageTest extends TestCase {
	...
	/** @return string[] Unique otsw_-prefixed option names written anywhere in the source. */
	private function writtenOptions(): array {
		$found = array();
		foreach ( $this->sourceFiles() as $file ) {
			$src = (string) file_get_contents( $file );
			// update_option / add_option / register_setting( group, 'otsw_...' )
			preg_match_all( "/(?:update_option|add_option)\(\s*'(otsw_[a-z0-9_]+)'/", $src, $m1 );
			preg_match_all( "/register_setting\(\s*'[^']+',\s*'(otsw_[a-z0-9_]+)'/", $src, $m2 );
	...
	public function test_transient_prefixes_cover_every_key_family_the_plugin_creates(): void {
		...
		$expected = array( 'otsw_v', 'otsw_rl_', 'otsw_nr_', 'otsw_batch_retry_' );
	...
	public function test_transient_key_families_still_exist_in_source(): void {
		...
		$this->assertStringContainsString( "'otsw_rl_'", $all_src );
		$this->assertStringContainsString( "'otsw_nr_'", $all_src );
		$this->assertStringContainsString( "'otsw_batch_retry_'", $all_src );
		$this->assertStringContainsString( "'otsw_v'", $all_src );
	}

	public function test_no_broad_prefix_deletes_anywhere_in_cleanup_code(): void {
		// The exact C2 bug: DELETE ... LIKE 'otsw_%' (or its transient forms)
		...
		$files[] = OTSW_PLUGIN_DIR . 'uninstall.php';

		foreach ( $files as $file ) {
			$src = (string) file_get_contents( $file );
			foreach ( array( "'otsw_' ) . '%'", "'_transient_otsw_' ) . '%'", "'_transient_timeout_otsw_' ) . '%'" ) as $dangerous ) {
				$this->assertStringNotContainsString(
					$dangerous,
					$src,
					basename( $file ) . ' contains a broad otsw_ prefix delete — this destroys WooCommerce Subscriptions data'
				);
			}
		}
	}
	...
	public function test_uninstall_uses_the_shared_lists(): void {
		$src = (string) file_get_contents( OTSW_PLUGIN_DIR . 'uninstall.php' );
	...
	private function dynamicCronHooksInSource(): array {
		...
		foreach ( $this->sourceFiles() as $file ) {
			$src = (string) file_get_contents( $file );
			preg_match_all( "/wp_schedule_single_event\(\s*[^,]+,\s*'(otsw_[a-z0-9_]+)'/", $src, $m );
	 ...
	 * otsw_retry_rebuild_scheduling/otsw_retry_product_enqueue/
	 * otsw_retry_product_delete all were before that mechanism existed.
	 */
	...
	public function test_otsw_rate_limits_table_is_dropped_on_uninstall(): void {
		...
		$uninstall = (string) file_get_contents( OTSW_PLUGIN_DIR . 'uninstall.php' );
		$this->assertStringContainsString( 'otsw_rate_limits', $uninstall );

		$admin = (string) file_get_contents( OTSW_PLUGIN_DIR . 'includes/class-admin-settings.php' );
		$this->assertStringContainsString( 'otsw_rate_limits', $admin );
	}
}
