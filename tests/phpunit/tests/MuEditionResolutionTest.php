...
/**
 * otsw_mu_resolve_active_edition() — which edition (if any) the MU
...
final class MuEditionResolutionTest extends TestCase {
	...
	public function test_free_active_only_resolves_to_the_free_directory(): void {
		$GLOBALS['otsw_test_active_plugins'] = array( self::FREE_BASENAME );

		$edition = otsw_mu_resolve_active_edition();

		$this->assertNotNull( $edition );
		$this->assertFalse( $edition['is_pro'] );
		$this->assertSame( WP_PLUGIN_DIR . '/ozulabs-turbo-search-for-woocommerce', $edition['dir'] );
	}
	...
	public function test_both_editions_active_is_unresolved(): void {
		...
		$GLOBALS['otsw_test_active_plugins'] = array( self::FREE_BASENAME, self::PRO_BASENAME );

		$this->assertNull( otsw_mu_resolve_active_edition() );
	}

	public function test_neither_edition_active_is_unresolved(): void {
		$GLOBALS['otsw_test_active_plugins'] = array( 'some-other-plugin/some-other-plugin.php' );
	...
	public function test_network_active_free_on_multisite_resolves_to_free(): void {
		...
		$GLOBALS['otsw_test_active_plugins'] = array(); // not site-active
	...
	public function test_neither_active_skips_the_fast_path_even_though_frees_own_files_exist_on_disk(): void {
		...
		// at WP_PLUGIN_DIR/ozulabs-turbo-search-for-woocommerce (the test
		// symlink), but with nothing active, the fast path must still skip
		// entirely.
		$GLOBALS['otsw_test_active_plugins'] = array();
	...
	public function test_selected_edition_missing_its_files_skips_fast_path_without_fatal(): void {
		...
		otsw_cache_bypass_intercept(); // must not throw/fatal

		$this->assertSame( array(), $GLOBALS['otsw_test_transients']['reads'] ?? array() );
	}
}
