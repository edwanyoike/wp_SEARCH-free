...
final class ActivatorTest extends TestCase {
	...
	private function healthyTables( string $create_suffix = '' ): void {
		$this->wpdb->handler = function ( string $sql, string $type ) use ( $create_suffix ) {
			...
			if ( 'row' === $type && str_contains( $sql, 'SHOW CREATE TABLE' ) ) {
				return array( 'wp_otsw_search_index', 'CREATE TABLE ... FULLTEXT KEY search_data (title,sku,content)' . $create_suffix );
			}
	...
	public function test_version_bump_runs_migration_and_stores_new_version(): void {
		update_option( 'otsw_db_version', '1.0.0' );
		update_option( 'otsw_mu_version', OTSW_VERSION ); // MU already current
		$this->healthyTables();

		Activator::init();

		$this->assertNotEmpty( $GLOBALS['otsw_test_dbdelta'], 'dbDelta must run on version bump' );
		$this->assertNotSame( '1.0.0', get_option( 'otsw_db_version' ) );
	}

	public function test_otsw_rate_limits_table_is_created_on_migration(): void {
		...
		$statements = implode( "\n", $GLOBALS['otsw_test_dbdelta'] );
		$this->assertStringContainsString( 'otsw_rate_limits', $statements );
	...
	public function test_current_version_skips_migration_and_table_probe_on_frontend(): void {
		...
		$current = get_option( 'otsw_db_version' );
	...
	public function test_fresh_activation_marker_does_not_trigger_init_rebuild(): void {
		update_option( 'otsw_db_version', '0' ); // never installed before... but stored '0' means fresh
		update_option( 'otsw_mu_version', OTSW_VERSION );
		$this->healthyTables();
	...
	public function test_parser_recorded_as_ngram_when_index_uses_ngram(): void {
		...
		$this->assertSame( 'ngram', get_option( 'otsw_ft_parser' ) );
	}
	...
	public function test_parser_recorded_as_default_without_ngram(): void {
		...
		$this->assertSame( 'default', get_option( 'otsw_ft_parser' ) );
	}
	...
	public function test_failed_table_creation_records_schema_error(): void {
		...
		$this->assertNotEmpty( get_option( 'otsw_schema_error' ) );
	}

	public function test_successful_creation_clears_a_previous_schema_error(): void {
		update_option( 'otsw_schema_error', 'old failure' );
		update_option( 'otsw_db_version', '1.0.0' );
		update_option( 'otsw_mu_version', OTSW_VERSION );
		$this->healthyTables();
		...
		$this->assertFalse( get_option( 'otsw_schema_error' ) );
	}
	...
	public function test_missing_innodb_records_a_clear_error_and_skips_create_table(): void {
		...
		$error = (string) get_option( 'otsw_schema_error' );
		$this->assertStringContainsString( 'InnoDB', $error );
		$this->assertSame( array(), $GLOBALS['otsw_test_dbdelta'], 'CREATE TABLE must never be attempted when InnoDB is unavailable' );
	}
	...
	public function test_innodb_support_disabled_records_a_clear_error(): void {
		...
		$this->assertStringContainsString( 'InnoDB', (string) get_option( 'otsw_schema_error' ) );
		$this->assertSame( array(), $GLOBALS['otsw_test_dbdelta'] );
	}
	...
	public function test_inconclusive_engine_check_fails_open_and_still_creates_tables(): void {
		...
		$this->assertNotEmpty( $GLOBALS['otsw_test_dbdelta'], 'table creation must proceed when the engine check itself is inconclusive' );
		$this->assertFalse( get_option( 'otsw_schema_error' ) );
	}
	...
	public function test_daily_gc_is_scheduled_once(): void {
		...
		Activator::init();
		$first = $GLOBALS['otsw_test_cron']['otsw_daily_transient_gc'] ?? null;
		Activator::init();

		$this->assertNotNull( $first );
		$this->assertSame( $first, $GLOBALS['otsw_test_cron']['otsw_daily_transient_gc'] );
	}
	 ...
	 * reliability work (otsw_retry_rebuild_scheduling, otsw_retry_product_
	 * enqueue, otsw_retry_product_delete) are all scheduled with dynamic,
	 * per-call arguments (a product ID, or a rebuild epoch/cursor pair) —
	 * unlike otsw_daily_transient_gc, wp_next_scheduled()/wp_unschedule_
	...
	public function test_clear_dynamic_cron_hooks_removes_every_pending_instance_regardless_of_args(): void {
		wp_schedule_single_event( time() + 30, 'otsw_retry_product_enqueue', array( 7 ) );
		wp_schedule_single_event( time() + 30, 'otsw_retry_product_enqueue', array( 8 ) );
		wp_schedule_single_event( time() + 30, 'otsw_retry_product_delete', array( 9 ) );
		wp_schedule_single_event( time() + 45, 'otsw_retry_rebuild_scheduling', array( 42, 500 ) );
	...
	public function test_clear_dynamic_cron_hooks_leaves_unrelated_hooks_alone(): void {
		...
		update_option( 'otsw_mu_version', OTSW_VERSION );
		$this->healthyTables();
		Activator::init(); // schedules otsw_daily_transient_gc
		wp_schedule_single_event( time() + 30, 'otsw_retry_product_enqueue', array( 7 ) );

		Activator::clear_dynamic_cron_hooks();

		$this->assertNotNull( $GLOBALS['otsw_test_cron']['otsw_daily_transient_gc'] ?? null, 'an unrelated recurring hook must survive' );
		$remaining = _get_cron_array();
		foreach ( $remaining as $hooks_at_timestamp ) {
			$this->assertArrayNotHasKey( 'otsw_retry_product_enqueue', $hooks_at_timestamp );
		}
	}
	...
	public function test_unrelated_active_plugins_do_not_trigger_detection(): void {
		$GLOBALS['otsw_test_active_plugins'] = array( 'woocommerce/woocommerce.php' );
	...
	public function test_each_network_site_processes_every_site_across_multiple_pages(): void {
		...
		$GLOBALS['otsw_test_all_site_ids'] = range( 1, $page_size + 5 ); // more than one page

		$visited = array();
		Activator::each_network_site( function () use ( &$visited ) {
			$visited[] = end( $GLOBALS['otsw_test_current_blog_stack'] );
		} );
	...
	public function test_each_network_site_restores_blog_context_after_every_site(): void {
		$GLOBALS['otsw_test_all_site_ids'] = array( 5, 6, 7 );

		Activator::each_network_site( static function () {} );

		$this->assertSame( array( 5, 6, 7 ), $GLOBALS['otsw_test_switched_blogs'] );
		$this->assertSame( 3, $GLOBALS['otsw_test_restored_blogs_count'], 'restore_current_blog() must run once per switch_to_blog()' );
		$this->assertSame( array(), $GLOBALS['otsw_test_current_blog_stack'], 'no blog context should remain switched after the loop finishes' );
	}
	...
	public function test_network_resource_check_detects_free_active_on_another_site(): void {
		$GLOBALS['otsw_test_is_multisite'] = true;
		$GLOBALS['otsw_test_all_site_ids'] = array( 1, 2, 3 );
		$free_basename                    = plugin_basename( OTSW_PLUGIN_DIR . 'turbo-search-for-woocommerce.php' );
		$GLOBALS['otsw_test_active_plugins'] = array( $free_basename );
	...
	public function test_network_resource_check_scans_every_site_and_returns_false_when_nowhere_active(): void {
		...
		$this->assertFalse( Activator::is_shared_network_resource_still_needed() );
		$this->assertSame( array( 1, 2, 3 ), $GLOBALS['otsw_test_switched_blogs'], 'must actually check every site before concluding nothing needs it' );
	}
	...
	public function test_each_network_site_restores_blog_context_even_when_callback_throws(): void {
		...
		$this->assertSame( 1, $GLOBALS['otsw_test_restored_blogs_count'] );
		$this->assertSame( array(), $GLOBALS['otsw_test_current_blog_stack'] );
	}
	...
	public function test_each_network_site_handles_an_empty_network(): void {
		...
		$this->assertSame( array(), $GLOBALS['otsw_test_switched_blogs'] );
	}
}
